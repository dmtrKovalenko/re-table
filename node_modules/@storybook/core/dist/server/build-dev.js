...
function buildDev(_ref) {
  ...
  var packageJson = _ref.packageJson,
      loadOptions = (0, _objectWithoutProperties3.default)(_ref, ['packageJson']);

  process.env.NODE_ENV = process.env.NODE_ENV || 'development';

  _commander2.default.version(packageJson.version).option('-p, --port [number]', 'Port to run Storybook (Required)', function (str) {
    return parseInt(str, 10);
  }).option('-h, --host [string]', 'Host to run Storybook').option('-s, --static-dir <dir-names>', 'Directory where to load static files from').option('-c, --config-dir [dir-name]', 'Directory where to load Storybook configurations from').option('--https', 'Serve Storybook over HTTPS. Note: You must provide your own certificate information.').option('--ssl-ca <ca>', 'Provide an SSL certificate authority. (Optional with --https, required if using a self-signed certificate)', _utils.parseList).option('--ssl-cert <cert>', 'Provide an SSL certificate. (Required with --https)').option('--ssl-key <key>', 'Provide an SSL key. (Required with --https)').option('--smoke-test', 'Exit after successful start').option('--quiet', 'Suppress verbose build output').option('-d, --db-path [db-file]', 'DEPRECATED!').option('--enable-db', 'DEPRECATED!').parse(process.argv);

  _nodeLogger.logger.info(_chalk2.default.bold(packageJson.name + ' v' + packageJson.version) + _chalk2.default.reset('\n'));
  ...
  if (!hasCustomFavicon) {
    app.use((0, _serveFavicon2.default)(defaultFavIcon));
  }
  ...
  app.use((0, _middleware2.default)(configDir, loadOptions, _commander2.default.quiet));
